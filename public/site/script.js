...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    function renderCards(container, data, type) {
        ...
        data.forEach(item => {
            ...
            const link = item.link; // Get External Link if available
            ...
            // If link exists, wrap image in anchor
            let imageHtml = `<img src="${image}" alt="${title}">`;
            if (link) {
                imageHtml = `<a href="${link}" target="_blank" style="display:block; cursor:pointer;">${imageHtml}</a>`;
            }

            // If link exists, wrap title in anchor
